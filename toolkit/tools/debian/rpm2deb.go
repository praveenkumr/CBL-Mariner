...
func createControlFile(pkg string) (int64, error) {
	content := "Source: rtd\n"
	...
	_, pkgName := getPackageName(strings.Split(pkg, "-"))
	...
	return doCreateAndDoWriteIO("control", 0644, strings.NewReader(content), pkg)
}
...
func createDebPkgMandatoryFiles(pkg string) error {
	_, err := createRulesFile(pkg)
	if err != nil {
		return err
	}

	_, err = createControlFile(pkg)
	if err != nil {
		return err
	}

	_, err = createChangelogFile(pkg)
	if err != nil {
		return err
	}

	_, err = createCompatFile(pkg)
	if err != nil {
		return err
	}
	return nil
...
func getDebFilesGenerated(pkgName string) error {
	...
	err = createDebPkgMandatoryFiles(pkgName)
	if err != nil {
		logger.Println(err)
		return err
	}
...
func convertSingleRPMFromURL(url string, toLocation string) error {
	 ...
	 */
	err = getDebFilesGenerated(pkgName)
	if err != nil {
